...
awk -F';' 'NR<=2 || ($3 ~ /^\([0-9]+,[0-9]+\)$/ && $5 ~ /^<[^,]+,[^,]+,[^,]+,[^,]+,[^,]+,[^,]+,[^,]+,[^,]+>$/) { print }' "simulation_results/test4_wrapped_grid_log.csv" > "simulation_results/test4_wrapped_grid_log.csv.tmp" && mv -f "simulation_results/test4_wrapped_grid_log.csv.tmp" "simulation_results/test4_wrapped_grid_log.csv"